...
<head>
    ...
    <title>{{__('msgLang.titPagInd')}}</title>
</head>
<body>
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-4">
                <form id="id-form-novo-pedido" class="form-group my-2" name="formPedido">
                    ...
                    <div class="row">
                        <div class="col-4">
                            <a class="btn btn-primary font-weight-bold w-100" href="{{route('home')}}">{{__('msgLang.btnVol')}}</a>
                        </div>
                        <div class="col-8">
                            <input type="submit" class="btn btn-info font-weight-bold w-100" value="{{__('msgLang.lblNvoPed')}}">
                        </div>
                    </div>
                </form>
                <div class="list-group listPedidosSizeVertical my-2" id="id-pedidos-list" role="tablist">
                    @foreach ($pedidos as $pedido)
                        <a class="list-group-item list-group-item-action" data-toggle="list" href="#" role="tab" value={{$pedido->id}}>{{__('msgLang.cmpOrd')}} {{$pedido->id}}</a>
                    @endforeach
                </div>
            </div>
            <div class="col-lg-4">
                <h3 class="my-2 text-center"><strong>{{__('msgLang.titPagAdi')}}</strong></h3>
                ...
                <form id="id-form-add-pedido-produto" method="post" action="">
                    @csrf
                    <input type="submit" id="id-botao-adicionar-produto" class="btn btn-success btn-block my-2 font-weight-bold" value="{{__('msgLang.lblAdiPrd')}}">    
                </form>
                ...
                <form id="id-form-enviar-pedido" method="post" action="">
                    @csrf
                    <input type="submit" id="id-botao-enviar-pedido" class="btn btn-info btn-block my-2 font-weight-bold" value="{{__('msgLang.lblEnvPrd')}}">
                </form>
            </div>
            <div class="col-lg-4">
                ...
                <div class="input-group">
                    <input type="text" class="form-control font-weight-bold" value="{{__('msgLang.lblVlrTot')}}:" disabled>
    ...
    <div class="modal fade" id="id-destroy-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <h5 class="modal-title" id="exampleModalLabel">{{__('msgLang.titPagAdi')}}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
            </div>
            <div class="modal-body">
                {{__('msgLang.msgRemPrd')}}
            </div>
            <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">{{__('msgLang.btnCan')}}</button>
